...
func TestScriptsWithVersion(t *testing.T) {
	var buf bytes.Buffer
	if err := ScriptsWithVersion("3.13.3", PluginFocus).Render(context.Background(), &buf); err != nil {
		t.Fatalf("Render error: %v", err)
	}
	got := buf.String()
...
func TestCloakCSS(t *testing.T) {
	var buf bytes.Buffer
	if err := CloakCSS().Render(context.Background(), &buf); err != nil {
		t.Fatalf("Render error: %v", err)
	}
	got := buf.String()
...
func TestScriptsWithNonce(t *testing.T) {
	...
	if err := ScriptsWithNonce(nonce, PluginFocus).Render(context.Background(), &buf); err != nil {
		t.Fatalf("Render error: %v", err)
	}
	got := buf.String()
